...
const UserList = () => {
  ...
  return (
    <section className="p-4 sm:p-6 bg-white min-h-screen max-w-full">
    ...
    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
      <div className="flex w-full max-w-md">
        <input
          ...
          placeholder="Search"
          className="border border-gray-300 rounded-l-md px-3 py-2 w-full focus:outline-none text-sm"
          onKeyDown={(event) => {
            if (event.key === 'Enter') {
              handleSearchClick();
            }
        ...
        <button
          ...
          className="bg-[#8B2323] text-white px-4 py-2 rounded-r-md hover:bg-red-900 transition shadow-md text-sm font-medium"
        >
          Search
        </button>
      </div>
  ...
  <div className="bg-white rounded-lg shadow-xl border border-gray-200">
    <div className="relative overflow-x-auto md:overflow-visible">
      <table className="min-w-[980px] w-full text-left table-auto">
        <thead className="bg-[#212529] text-white whitespace-nowrap">
          <tr>
            <th className="px-3 py-3 text-xs sm:text-sm font-semibold">ID</th>
            <th className="px-3 py-3 text-xs sm:text-sm font-semibold">FullName</th>
            <th className="px-3 py-3 text-xs sm:text-sm font-semibold">Email</th>
            <th className="px-3 py-3 text-xs sm:text-sm font-semibold">Gender</th>
        ...
        <tbody className="divide-y divide-gray-100 whitespace-nowrap">
          {isLoading ? (
            <tr>
              <td colSpan="7" className="p-10 text-center text-gray-500 text-sm">Loading...</td>
            </tr>
          ) : users.length === 0 ? (
            <tr>
              <td colSpan="7" className="p-10 text-center text-gray-500 text-sm">No users found.</td>
            </tr>
          ) : (
  ...
  <div className="flex flex-col lg:flex-row items-center justify-between gap-4 mt-6">
    <div className="flex items-center gap-2 text-xs sm:text-sm font-medium order-2 lg:order-1">
      ...
      <select
        ...
        className="border border-gray-300 rounded px-2 py-1 outline-none focus:border-red-500 bg-white"
      >
        {[5, 10, 20, 50].map((size) => (
